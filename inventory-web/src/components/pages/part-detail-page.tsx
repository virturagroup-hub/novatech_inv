    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 pt-4 sm:px-6 lg:px-8 lg:pt-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
          ...
          <CardContent className="space-y-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0 space-y-2">
                <p className="font-mono text-sm font-semibold text-white">{getDisplayPartNumber(part)}</p>
                <p className="text-lg font-semibold text-slate-100">{part.partName}</p>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="rounded-3xl border border-white/10 bg-slate-950/60 p-4">
                ...
                <p className="mt-2 text-3xl font-semibold text-white">{part.quantityOnHand}</p>
                <p className="mt-1 text-sm text-slate-400">
                  Reorder point {part.reorderPoint} · Target {part.reorderTarget}
                </p>
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  <div className="rounded-2xl border border-white/10 bg-slate-950/70 p-3">
                    <p className="text-[11px] uppercase tracking-[0.22em] text-slate-500">Received</p>
                    <p className="mt-2 font-mono text-xs text-slate-100">{formatDateTime(part.receivedAt)}</p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-slate-950/70 p-3">
                    <p className="text-[11px] uppercase tracking-[0.22em] text-slate-500">Updated</p>
                    <p className="mt-2 font-mono text-xs text-slate-100">{formatDateTime(part.updatedAt)}</p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-slate-950/70 p-3">
                    <p className="text-[11px] uppercase tracking-[0.22em] text-slate-500">Counted</p>
                    <p className="mt-2 font-mono text-xs text-slate-100">{formatDateTime(part.lastCountedAt)}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <Card className="border-white/10 bg-white/5">
          ...
          <CardContent>
            <div className="rounded-3xl border border-white/10 bg-slate-950/60 p-4">
              <p className="text-sm leading-6 text-slate-200">{part.notes || "No notes on file."}</p>
            </div>
          </CardContent>
        </Card>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-3 flex flex-wrap gap-2 text-[11px] uppercase tracking-[0.22em] text-slate-500">
                          {entry.audit?.delta !== undefined && (
                            <span>Delta {entry.audit.delta}</span>
                          )}
                          {entry.audit?.nextQuantity !== undefined && (
                            <span>After {entry.audit.nextQuantity}</span>
                          )}
                          {entry.audit?.nextLocationId && <span>Location updated</span>}
                        </div>
                      </div>
                    );
                  })}
