        ...
        {!labels.length ? (
          ...
          <div className="space-y-6 print:space-y-0">
            {Array.from({ length: sheetCount }, (_, sheetIndex) => {
              ...
              const breakAfterSheet = sheetIndex < sheetCount - 1;

              return (
                <section
                  key={sheetIndex}
                  className="print-sheet overflow-hidden rounded-[0.75rem] border border-slate-300 bg-white p-2 shadow-lg shadow-slate-200/60 print:rounded-none print:border-0 print:p-0 print:shadow-none"
                  style={{
                    breakAfter: breakAfterSheet ? "page" : "auto",
                    pageBreakAfter: breakAfterSheet ? "always" : "auto",
                  }}
                >
  ...
  return (
    <div className="flex h-full min-h-0 flex-col items-center justify-center gap-2 overflow-hidden rounded-xl border border-slate-300 bg-white p-2 text-slate-950 print:border-slate-300">
      ...
      <div className="space-y-0.5 text-center">
        <p className="font-mono text-[13px] font-semibold leading-4">{part.partNumber}</p>
...
function EmptyLabelSlot() {
  return (
    <div
      aria-hidden="true"
      className="h-full min-h-0 rounded-xl border border-dashed border-slate-200 bg-slate-50/40 print:border-0 print:bg-transparent"
    />
  );
}
